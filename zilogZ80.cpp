...
{
    uint8_t value = cpu->IOPorts[port];
    switch (port)
    {
    case 3:
        value = (cpu->shiftRegister >> (8 - cpu->shiftOffset)) & 0xFF;
        break;
    case 0x10:
        if (g_serialState && g_serialState->serial.connected() && !g_serialState->serial.rxBuf.empty())
        {
            value = g_serialState->serial.rxBuf.front();
...
static void Z80WritePort(zilogZ80 *cpu, uint8_t port, uint8_t value)
{
    ...
    case 4:
    {
        ...
        cpu->shiftRegister |= data >> cpu->shiftOffset;
        break;
    }
    case 0x10:
        if (g_serialState && g_serialState->serial.connected())
            g_serialState->serial.txBuf.push_back(value);
        break;
    case 0x11:
        break;
    }
...
static void Z80ProcessDDCB(zilogZ80 *cpu, uint16_t addr)
{
    ...
    if (op < 0x40)
    {
        switch ((op >> 3) & 0x07)
        {
        case 0: result = Z80RLC(value, carry); break;
        case 1: result = Z80RRC(value, carry); break;
        case 2: result = Z80RL(value, cpu->cf, carry); break;
        case 3: result = Z80RR(value, cpu->cf, carry); break;
        case 4: result = Z80SLA(value, carry); break;
        case 5: result = Z80SRA(value, carry); break;
        case 6: result = Z80SLL(value, carry); break;
        case 7: result = Z80SRL(value, carry); break;
        }
...
static void Z80PrefixDDFD(zilogZ80 *cpu, uint16_t &IR)
{
    ...
    {
    // ADD IR, rr
    case 0x09: Z80AddIR(cpu, IR, GetBC(cpu)); return;
    case 0x19: Z80AddIR(cpu, IR, GetDE(cpu)); return;
    case 0x29: Z80AddIR(cpu, IR, IR);         return;
    case 0x39: Z80AddIR(cpu, IR, cpu->SP);    return;

    // LD IR, nn
    case 0x21: IR = AdvanceWord(cpu); return;
    ...
    case 0x23: IR++; return;  // INC IR
    case 0x2B: IR--; return;  // DEC IR

    // INC/DEC IRH/IRL (undocumented)
    case 0x24: Z80SetIRH(IR, INR(cpu, Z80IRH(IR))); return;
    case 0x25: Z80SetIRH(IR, DCR(cpu, Z80IRH(IR))); return;
    case 0x26: Z80SetIRH(IR, AdvanceByte(cpu));      return;
    case 0x2C: Z80SetIRL(IR, INR(cpu, Z80IRL(IR))); return;
    case 0x2D: Z80SetIRL(IR, DCR(cpu, Z80IRL(IR))); return;
    case 0x2E: Z80SetIRL(IR, AdvanceByte(cpu));      return;

    // LD IR, (nn)
    case 0x2A:
    {
        uint16_t addr = AdvanceWord(cpu);
        IR = cpu->memory[addr] | ((uint16_t)cpu->memory[addr + 1] << 8);
        return;
    }
    ...
    case 0x34:
    {
        ...
        cpu->WriteMem(addr, INR(cpu, cpu->memory[addr]));
        return;
    }
    ...
    {
        uint16_t addr = IR + (int8_t)AdvanceByte(cpu);
        cpu->WriteMem(addr, DCR(cpu, cpu->memory[addr]));
        return;
    }
    ...
    case 0x44: cpu->B = Z80IRH(IR); return;
    case 0x45: cpu->B = Z80IRL(IR); return;
    case 0x46: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->B = cpu->memory[a]; return; }
    case 0x4C: cpu->C = Z80IRH(IR); return;
    case 0x4D: cpu->C = Z80IRL(IR); return;
    case 0x4E: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->C = cpu->memory[a]; return; }
    case 0x54: cpu->D = Z80IRH(IR); return;
    case 0x55: cpu->D = Z80IRL(IR); return;
    case 0x56: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->D = cpu->memory[a]; return; }
    case 0x5C: cpu->E = Z80IRH(IR); return;
    case 0x5D: cpu->E = Z80IRL(IR); return;
    case 0x5E: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->E = cpu->memory[a]; return; }

    // LD IRH, r / LD H, (IR+d) (undocumented)
    case 0x60: Z80SetIRH(IR, cpu->B);          return;
    case 0x61: Z80SetIRH(IR, cpu->C);          return;
    case 0x62: Z80SetIRH(IR, cpu->D);          return;
    case 0x63: Z80SetIRH(IR, cpu->E);          return;
    case 0x64: return; // LD IRH, IRH — NOP
    case 0x65: Z80SetIRH(IR, Z80IRL(IR));      return;
    case 0x66: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->H = cpu->memory[a]; return; }
    case 0x67: Z80SetIRH(IR, cpu->A);          return;

    // LD IRL, r / LD L, (IR+d) (undocumented)
    case 0x68: Z80SetIRL(IR, cpu->B);          return;
    case 0x69: Z80SetIRL(IR, cpu->C);          return;
    case 0x6A: Z80SetIRL(IR, cpu->D);          return;
    case 0x6B: Z80SetIRL(IR, cpu->E);          return;
    case 0x6C: Z80SetIRL(IR, Z80IRH(IR));      return;
    case 0x6D: return; // LD IRL, IRL — NOP
    case 0x6E: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->L = cpu->memory[a]; return; }
    case 0x6F: Z80SetIRL(IR, cpu->A);          return;

    // LD (IR+d), r
    case 0x70: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->WriteMem(a, cpu->B); return; }
    case 0x71: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->WriteMem(a, cpu->C); return; }
    case 0x72: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->WriteMem(a, cpu->D); return; }
    case 0x73: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->WriteMem(a, cpu->E); return; }
    case 0x74: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->WriteMem(a, cpu->H); return; }
    case 0x75: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->WriteMem(a, cpu->L); return; }
    case 0x77: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->WriteMem(a, cpu->A); return; }

    // LD A, IRH / LD A, IRL / LD A, (IR+d)
    case 0x7C: cpu->A = Z80IRH(IR); return;
    case 0x7D: cpu->A = Z80IRL(IR); return;
    case 0x7E: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); cpu->A = cpu->memory[a]; return; }

    // ADD A / ADC A / SUB / SBC / AND / XOR / OR / CP — IRH, IRL, (IR+d)
    case 0x84: AddReg(cpu, &cpu->A, Z80IRH(IR), 0);       return;
    case 0x85: AddReg(cpu, &cpu->A, Z80IRL(IR), 0);       return;
    case 0x86: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); AddReg(cpu, &cpu->A, cpu->memory[a], 0);       return; }
    case 0x8C: AddReg(cpu, &cpu->A, Z80IRH(IR), cpu->cf); return;
    case 0x8D: AddReg(cpu, &cpu->A, Z80IRL(IR), cpu->cf); return;
    case 0x8E: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); AddReg(cpu, &cpu->A, cpu->memory[a], cpu->cf); return; }
    case 0x94: SubReg(cpu, &cpu->A, Z80IRH(IR), 0);       return;
    case 0x95: SubReg(cpu, &cpu->A, Z80IRL(IR), 0);       return;
    case 0x96: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); SubReg(cpu, &cpu->A, cpu->memory[a], 0);       return; }
    case 0x9C: SubReg(cpu, &cpu->A, Z80IRH(IR), cpu->cf); return;
    case 0x9D: SubReg(cpu, &cpu->A, Z80IRL(IR), cpu->cf); return;
    case 0x9E: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); SubReg(cpu, &cpu->A, cpu->memory[a], cpu->cf); return; }
    case 0xA4: ANA(cpu, &cpu->A, Z80IRH(IR)); return;
    case 0xA5: ANA(cpu, &cpu->A, Z80IRL(IR)); return;
    case 0xA6: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); ANA(cpu, &cpu->A, cpu->memory[a]); return; }
    case 0xAC: XRA(cpu, &cpu->A, Z80IRH(IR)); return;
    case 0xAD: XRA(cpu, &cpu->A, Z80IRL(IR)); return;
    case 0xAE: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); XRA(cpu, &cpu->A, cpu->memory[a]); return; }
    case 0xB4: ORA(cpu, &cpu->A, Z80IRH(IR)); return;
    case 0xB5: ORA(cpu, &cpu->A, Z80IRL(IR)); return;
    case 0xB6: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); ORA(cpu, &cpu->A, cpu->memory[a]); return; }
    case 0xBC: CMP(cpu, cpu->A, Z80IRH(IR)); return;
    case 0xBD: CMP(cpu, cpu->A, Z80IRL(IR)); return;
    case 0xBE: { uint16_t a = IR + (int8_t)AdvanceByte(cpu); CMP(cpu, cpu->A, cpu->memory[a]); return; }
    ...
    {
        uint8_t lo = cpu->memory[cpu->SP];
        uint8_t hi = cpu->memory[cpu->SP + 1];
    ...
    // JP (IR)
    case 0xE9: cpu->PC = IR; return;

    // LD SP, IR
    case 0xF9: cpu->SP = IR; return;
...
void ExecuteZ80OpCode(uint8_t OpCode, zilogZ80 *cpu)
{
    if (cpu->prefixState != Z80Prefix::None)
    {
        ...
        {
        case Z80Prefix::CB:
            Z80ProcessCB(cpu->memory[cpu->PC], cpu);
            cpu->PC += 1;
            break;
        case Z80Prefix::ED:
            Z80PrefixED(cpu);
    ...
    case 0x09: cpu->PC += 1; Z80AddHLPlain(cpu, GetBC(cpu));    return;
    case 0x19: cpu->PC += 1; Z80AddHLPlain(cpu, GetDE(cpu));    return;
    case 0x29: cpu->PC += 1; Z80AddHLPlain(cpu, Z80GetHL(cpu)); return;
    case 0x39: cpu->PC += 1; Z80AddHLPlain(cpu, cpu->SP);       return;
    ...
    case 0x27:
    {
        ...
        int a = cpu->A;
        if (!cpu->nf) {
            if (cpu->acf || (a & 0x0F) > 9) a += 6;
            if (cpu->cf  || a > 0x9F)        a += 0x60;
        } else {
            if (cpu->acf) a -= 6;
            if (cpu->cf)  a -= 0x60;
        }
        ...
        cpu->acf = (old_a ^ cpu->A) & 0x10; // H = bit 4 changed
        cpu->Set_SZP_Flags(cpu->A);
        // nf unchanged
        return;
    }
    ...
    case 0x08:
    {
        // EX AF, AF'
        cpu->PC += 1;
        uint8_t tmpA = cpu->A;  cpu->A  = cpu->A_;  cpu->A_  = tmpA;
        bool tmpSF = cpu->sf;   cpu->sf  = cpu->sf_;  cpu->sf_  = tmpSF;
        bool tmpZF = cpu->zf;   cpu->zf  = cpu->zf_;  cpu->zf_  = tmpZF;
        bool tmpACF = cpu->acf; cpu->acf = cpu->acf_; cpu->acf_ = tmpACF;
        bool tmpPF = cpu->pf;   cpu->pf  = cpu->pf_;  cpu->pf_  = tmpPF;
        bool tmpCF = cpu->cf;   cpu->cf  = cpu->cf_;  cpu->cf_  = tmpCF;
        bool tmpYF = cpu->yf;   cpu->yf  = cpu->yf_;  cpu->yf_  = tmpYF;
        bool tmpXF = cpu->xf;   cpu->xf  = cpu->xf_;  cpu->xf_  = tmpXF;
        bool tmpNF = cpu->nf;   cpu->nf  = cpu->nf_;  cpu->nf_  = tmpNF;
        return;
    }
    ...
    case 0xD9:
    {
        ...
        cpu->PC += 1;
        uint8_t tmpB = cpu->B; cpu->B = cpu->B_; cpu->B_ = tmpB;
        uint8_t tmpC = cpu->C; cpu->C = cpu->C_; cpu->C_ = tmpC;
        uint8_t tmpD = cpu->D; cpu->D = cpu->D_; cpu->D_ = tmpD;
        uint8_t tmpE = cpu->E; cpu->E = cpu->E_; cpu->E_ = tmpE;
        uint8_t tmpH = cpu->H; cpu->H = cpu->H_; cpu->H_ = tmpH;
        uint8_t tmpL = cpu->L; cpu->L = cpu->L_; cpu->L_ = tmpL;
        return;
    }
    ...
    default:
        ...
        if ((OpCode & 0xC0) == 0x80) {
            ...
            switch (OpCode) {
            case 0xC6: case 0xCE:                   cpu->nf = false; break; // ADD A,n; ADC A,n
            case 0xD6: case 0xDE:                   cpu->nf = true;  break; // SUB n; SBC A,n
            case 0xE6: case 0xEE: case 0xF6:        cpu->nf = false; break; // AND/XOR/OR n
            case 0xFE:                              cpu->nf = true;  break; // CP n
            default: break;
            }
        }
        return;
